# Nuxt SSR DevTools — Chrome extension

DevTools panel that lists Nuxt SSR fetch calls captured by
[`nuxt-ssr-devtools`](../server).
...
4. Open a Nuxt dev/staging page that has `nuxt-ssr-devtools` set up
5. Open DevTools (F12) → **Nuxt SSR Fetches** tab
...
Output: `dist/nuxt-ssr-devtools-v<version>.zip` containing only the runtime
files (manifest, devtools/panel HTML/JS/CSS, three icon PNGs). Submit this at
